...
class SINK_PQ(object):
    ...
    """
    def __init__(self, M, gamma_val, Ks=256, verbose=True):
        assert 0 < Ks <= 2 ** 32
    ...
    def cdist(self, A, B):
        ...
        for i in range(A.shape[0]):
            for j in range(B.shape[0]):
                mat[i,j] = SINK(A[i], B[j], gamma_val=self.gamma_val)
        return mat
    ...
    def dtable(self, query):
        ...
        for m in range(self.M):
            ...
            for j in range(self.Ks):
                dtable[m, j] = SINK(query_sub, self.codewords[m, j], gamma_val = self.gamma_val)
