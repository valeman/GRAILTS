...
class NCC_PQ(object):
    ...
    def fit(self, vecs, iter=20, seed=123):
        ...
        self.indices = np.zeros((N,self.M))
        ...
        for m in range(self.M):
            ...
            self.indices[:,m], self.codewords[m] = matlab_kshape(vecs_sub, self.Ks)
      #  print("codewords:")
       # print(self.codewords[0])
    ...
    def vq(self,obs, code_book):
        ...
        dist = self.cdist(obs, code_book)
        code = dist.argmax(axis=1)
        return code
    ...
    #     #     vecs_sub = vecs[:, m * self.Ds : (m+1) * self.Ds]
    #     #     codes[:, m] =   #self.vq(vecs_sub, self.codewords[m])
    #
    #     return self.indices.astype(int)
    ...
    def encode(self, vecs):
        ...
        print('codes:')
        print(codes)
        print(max(NCC(self.codewords[0,codes[0,0]], vecs[0,0:8])))
        print(max(NCC(self.codewords[0,codes[1,0]], vecs[0,0:8])))
        print(max(NCC(self.codewords[0,codes[1, 0]], vecs[1, 0:8])))
        print(max(NCC(self.codewords[0,codes[0, 0]], vecs[1, 0:8])))
        return codes #self.indices.astype(int)
    ...
    def dtable(self, query):
        ...
        for m in range(self.M):
            ...
            for j in range(self.Ks):
                dtable[m, j] = max(NCC(query_sub, self.codewords[m, j]))

        return DistanceTable(dtable)

    # def adist(self, query_index, codes):
    #     N, M = codes.shape
    ...
    #     for n in range(N):
    #         for m in range(M):
    #             if codes[query_index, m] == codes[n, m]:
    #                 dists[n] += 1
    #
    #     return dists
...
class DistanceTable(object):
    ...
    def adist(self, codes):
        ...
        # dists = np.max(self.dtable[range(M), codes], axis=1)

        # dists = np.zeros((N, )).astype(np.float32)
        # for n in range(N):
        #     for m in range(M):
        #         dists[n] = max(dists[n], self.dtable[m][codes[n][m]])
