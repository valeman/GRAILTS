...
double snorm(fftw_complex* x, int xlen)
{
	int i;
	double sum = 0;
	for (i = 0; i < xlen; i++)
...
fftw_complex* NCCb(fftw_complex* x,int xlen, fftw_complex* y,int ylen)
{
	int i;
...
fftw_complex* NCC(fftw_complex* x, int xlen, fftw_complex* y, int ylen)
{
	int i;
	int len = (int)pow(2, ceil(log2(abs(2 * max(xlen, ylen) - 1))));
