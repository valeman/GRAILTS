...
def CIDdtw(x,y,constraint=None,w=5):
    
    if (constraint == "None"):
        return ciddtw_n(x,y)/CID(x,y);
    elif constraint == "Sakoe-Chiba":
        return ciddtw_scb(x,y,w)/CID(x,y);
    elif constraint == "Itakura":
        return ciddtw_ip(x,y,w)/CID(x,y);
    else:
